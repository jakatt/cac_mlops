...
```
ETL → validation schéma → train → sélection champion → GATE MANUELLE → promote → Kapsule
```
...
```
build images → git pull VPS → docker compose up → smoke test → GATE MANUELLE → Kapsule
```
...
extract blueprint → train avec nouveaux params → compare @Production
  → si meilleur : GATE MANUELLE → promote → Kapsule
...
4. Cliquer **Resume** pour valider → promote @Production → deploy Kapsule
...
| Drift share | Niveau | Action automatique |
|---|---|---|
| < 10 % | OK | Rien |
| 10–25 % | WARNING | Email d'alerte, monitoring renforcé |
| > 25 % | CRITICAL | Email + `retrain-annual` déclenché automatiquement |

Si `retrain-annual` est déclenché par drift CRITICAL → valider la gate dans Prefect UI comme pour le trigger 1.
...
| Drift CRITICAL email reçu | `retrain-annual` déclenché auto | Valider la gate dans Prefect UI |
