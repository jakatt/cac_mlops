...
_model   = None
_df      = None   # features only (What-If)
_df_full = None   # features + grav réelle (heatmap)
...
def _get_data() -> pd.DataFrame | None:
    """Features seules — utilisées par le What-If (prédiction nécessaire)."""
    global _df
    if _df is not None:
        return _df
    candidates = [
...
def _get_data_with_labels() -> pd.DataFrame | None:
    """Features + gravité réelle — utilisées par la heatmap (pas de prédiction)."""
    global _df_full
    ...
    candidates = [
        ...
        (DATA_ROOT / "cumul_2021_2022" / "X_test.csv",
         DATA_ROOT / "cumul_2021_2022" / "y_test.csv"),
        (DATA_ROOT / "2023" / "X_test.csv",
         DATA_ROOT / "2023" / "y_test.csv"),
        (DATA_ROOT / "2022" / "X_test.csv",
         DATA_ROOT / "2022" / "y_test.csv"),
    ]
...
def run_heatmap(
    ...
    # Agrégation par cellule ~500 m (arrondi à 0.005° ≈ 500 m)
    df["lat_r"] = (df["lat"] * 200).round() / 200
    df["lon_r"] = (df["long"] * 200).round() / 200

    agg = (
        df.groupby(["lat_r", "lon_r"])
        .agg(nb_accidents=("grav", "count"),
             pct_graves=("grav", "mean"))
        .reset_index()
    )
...
Représentation cartographique de la gravité **réelle** des accidents ONISR, agrégée par cellule géographique (~500 m).
""")
            with gr.Row():
            ...
            top_table = gr.Dataframe(
                ...
                headers=["Latitude", "Longitude", "Nb accidents", "% graves réel"],
                interactive=False,
            )
    ...
    gr.Markdown("""
---
*Modèle : Random Forest entraîné sur données ONISR 2021–2023 — [MLflow](http://mlflow:5000)*
*Onglet What-If : projection prédictive (non causale) — Onglet Points Noirs : données réelles ONISR*
""")
