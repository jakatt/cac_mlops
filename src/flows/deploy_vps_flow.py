...
def revert_blueprint_task(sha_tag: str) -> None:
    ...
    Logique de revert elle-même factorisée dans src/utils/blueprint_revert.py,
    partagée avec services/gradio/app.py::cancel_run (cas STOP manuel au gate).
    """
    from src.utils.blueprint_revert import revert_blueprint_on_main

    log = get_run_logger()
    revert_blueprint_on_main(
        sha_tag, reason="rollback automatique — test-api KO après promotion", log=log,
    )
